### useContext
> 作用：在接收 context 传递数据的组件中，使用 useContext 快速拿到数据
```jsx
-- context.js
...
```
### useReducer
> 组件中有大量数据,使用 useState,略显繁琐,使用 useReducer 更方便的管理数据
```js
...
```
### React.memo
> 作用类似于类组件的 PureComponent, 当父组件更新时,动态判断子组件使用更新
>
